...
func generateService(g *protogen.GeneratedFile, service *protogen.Service, file *protogen.File) error {
	...
	g.P("func New", service.GoName, "MCPServer(srv ", service.GoName, "Server) *", service.GoName, "MCPServer {")
	g.P("	s := &", service.GoName, "MCPServer{")
	g.P("		server: srv,")
	...
	g.P("func (s *", service.GoName, "MCPServer) Start() error {")
...
func generateMethodTool(g *protogen.GeneratedFile, service *protogen.Service, method *protogen.Method, index int) error {
	...
	isMCPMethod := proto.GetExtension(method.Desc.Options(), mcp.E_McpTool).(bool)
	if !isMCPMethod {
		return nil
	}

	// Get MCP method options
	mcpDescription := proto.GetExtension(method.Desc.Options(), mcp.E_McpToolDescription).(string)
